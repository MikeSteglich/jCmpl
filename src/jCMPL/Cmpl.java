 ...
 * @version 2.0   license LGPLv3
 */
public class Cmpl extends Thread {

    public static final int COMPATIBILITY = 3; // 2 CMPL 1.10 // 3 CMPL 2 
    ...
    public void solve() throws CmplException {
        ...
        if (_remoteMode) {
            ...
            String  cmplSuffix = "_cmpl__" + _id;

            _problem = _model.substring(0, _model.lastIndexOf("."));
            _cmplFile = _problem + cmplSuffix + ".cmpl";
            _cmplDataFile = _problem + cmplSuffix +".cdat";
            _cmplMsgFile = _problem + cmplSuffix + ".cmsg";
            _cmplSolFile = _problem + cmplSuffix + ".csol";
            ...
            Path source = Paths.get(_model);
            Path destination = Paths.get(_cmplFile);
            ...
            if (cmplBinFile.exists() && cmplBinFile.canExecute()) {
                ...
                if (_cmplProc.exitValue() != 0) {
                    throw new CmplException(_outputString.toString());
                }
    ...
    public void saveSolution(String solFileName) throws CmplException {
        ...
        if (_solutions.nrOfSolutions() > 0) {
            ...
            if (!_remoteMode) {
                solStr=new StringBuilder();

                for (String line : _solutions.solFileContent()) {
                    if (line.contains("instanceName")) {
                        line = "       <instanceName>"+_model+"</instanceName>";
                    }
                    solStr.append(line + "\n");
                }
                _solutionString=solStr.toString();
            }
    ...
    private void cleanUp() throws CmplException {
        ...
        if (!_isCleaned) {
            ...
            CmplTools.delTmpFile(_cmplFile);
        ...
        if (_solutions.nrOfSolutions() > 0) {
            ...
            for (CmplSolElement e : solObj) {
                if (e.name().startsWith(name)) {
                    if (e.name().contains("[")) {
                        if ( e.name().substring(0, e.name().indexOf("[")).equals(name) ) {
                            if (!isArray) {
                                isArray = true;
                            }
